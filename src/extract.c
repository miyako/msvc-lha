...
int extract_archive(LHAFilter *filter, LHAOptions *options, void (*_PA_YieldAbsolute)(void), void (*_cb)(char *, void *), void *_arr)
{
	int result;
	...
	for (;;) {
		...
		if (!extract_archived_file(filter->reader, header, options, _cb, _arr)) {
			result = 0;
		}
	}
